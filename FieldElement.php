...
final class FieldElement {
	...
	public function add(FieldElement $value): void {
		...
		$greaterThanOrEqualToPrimeMask = -((int)(((int)($this->limbs[0] < 0xFFFFFC2F) | (($this->limbs[1] ^ 0xFFFFFFFE) & 0xFFFFFFFE) | ($this->limbs[2] ^ 0xFFFFFFFF) | ($this->limbs[3] ^ 0xFFFFFFFF) | ($this->limbs[4] ^ 0xFFFFFFFF) | ($this->limbs[5] ^ 0xFFFFFFFF) | ($this->limbs[6] ^ 0xFFFFFFFF) | ($this->limbs[7] ^ 0xFFFFFFFF)) === 0) | ($carry >> 32));
		$lessThanPrimeMask = ~$greaterThanOrEqualToPrimeMask;
	...
	public function multiply(FieldElement $value): void {
		...
		$carryOne = $this->buffer[8] * 0x7FFFFFFF + $this->buffer[0];
		$this->buffer[0] = $carryOne & 0xFFFFFFFF;
		$carryOne = ($carryOne >> 32) + $this->buffer[9] * 0x7FFFFFFF + $this->buffer[1];
		$this->buffer[1] = $carryOne & 0xFFFFFFFF;
		$carryOne = ($carryOne >> 32) + $this->buffer[10] * 0x7FFFFFFF + $this->buffer[2];
		$this->buffer[2] = $carryOne & 0xFFFFFFFF;
		$carryOne = ($carryOne >> 32) + $this->buffer[11] * 0x7FFFFFFF + $this->buffer[3];
		$this->buffer[3] = $carryOne & 0xFFFFFFFF;
		$carryOne = ($carryOne >> 32) + $this->buffer[12] * 0x7FFFFFFF + $this->buffer[4];
		$this->buffer[4] = $carryOne & 0xFFFFFFFF;
		$carryOne = ($carryOne >> 32) + $this->buffer[13] * 0x7FFFFFFF + $this->buffer[5];
		$this->buffer[5] = $carryOne & 0xFFFFFFFF;
		$carryOne = ($carryOne >> 32) + $this->buffer[14] * 0x7FFFFFFF + $this->buffer[6];
		$this->buffer[6] = $carryOne & 0xFFFFFFFF;
		$carryOne = ($carryOne >> 32) + $this->buffer[15] * 0x7FFFFFFF + $this->buffer[7];
		$this->buffer[7] = $carryOne & 0xFFFFFFFF;
		$carryTwo = $this->buffer[8] * 0x7FFFFFFF + $this->buffer[0];
		$this->buffer[0] = $carryTwo & 0xFFFFFFFF;
		$carryTwo = ($carryTwo >> 32) + $this->buffer[9] * 0x7FFFFFFF + $this->buffer[1];
		$this->buffer[1] = $carryTwo & 0xFFFFFFFF;
		$carryTwo = ($carryTwo >> 32) + $this->buffer[10] * 0x7FFFFFFF + $this->buffer[2];
		$this->buffer[2] = $carryTwo & 0xFFFFFFFF;
		$carryTwo = ($carryTwo >> 32) + $this->buffer[11] * 0x7FFFFFFF + $this->buffer[3];
		$this->buffer[3] = $carryTwo & 0xFFFFFFFF;
		$carryTwo = ($carryTwo >> 32) + $this->buffer[12] * 0x7FFFFFFF + $this->buffer[4];
		$this->buffer[4] = $carryTwo & 0xFFFFFFFF;
		$carryTwo = ($carryTwo >> 32) + $this->buffer[13] * 0x7FFFFFFF + $this->buffer[5];
		$this->buffer[5] = $carryTwo & 0xFFFFFFFF;
		$carryTwo = ($carryTwo >> 32) + $this->buffer[14] * 0x7FFFFFFF + $this->buffer[6];
		$this->buffer[6] = $carryTwo & 0xFFFFFFFF;
		$carryTwo = ($carryTwo >> 32) + $this->buffer[15] * 0x7FFFFFFF + $this->buffer[7];
		$this->buffer[7] = $carryTwo & 0xFFFFFFFF;
		$carryThree = $this->buffer[8] * 0x000003D3 + $this->buffer[0];
		$this->buffer[0] = $carryThree & 0xFFFFFFFF;
		$carryThree = ($carryThree >> 32) + $this->buffer[9] * 0x000003D3 + $this->buffer[1];
		$this->buffer[1] = $carryThree & 0xFFFFFFFF;
		$carryThree = ($carryThree >> 32) + $this->buffer[10] * 0x000003D3 + $this->buffer[2];
		$this->buffer[2] = $carryThree & 0xFFFFFFFF;
		$carryThree = ($carryThree >> 32) + $this->buffer[11] * 0x000003D3 + $this->buffer[3];
		$this->buffer[3] = $carryThree & 0xFFFFFFFF;
		$carryThree = ($carryThree >> 32) + $this->buffer[12] * 0x000003D3 + $this->buffer[4];
		$this->buffer[4] = $carryThree & 0xFFFFFFFF;
		$carryThree = ($carryThree >> 32) + $this->buffer[13] * 0x000003D3 + $this->buffer[5];
		$this->buffer[5] = $carryThree & 0xFFFFFFFF;
		$carryThree = ($carryThree >> 32) + $this->buffer[14] * 0x000003D3 + $this->buffer[6];
		$this->buffer[6] = $carryThree & 0xFFFFFFFF;
		$carryThree = ($carryThree >> 32) + $this->buffer[15] * 0x000003D3 + $this->buffer[7];
		$this->buffer[7] = $carryThree & 0xFFFFFFFF;
		$oneThird = (($carryOne >> 32) + ($carryTwo >> 32) + ($carryThree >> 32)) * 0x5555569B;
		$carry = $oneThird + $this->buffer[0];
		$this->buffer[0] = $carry & 0xFFFFFFFF;
		...
		$this->buffer[6] = $carry & 0xFFFFFFFF;
		$this->buffer[7] = (($carry >> 32) + $this->buffer[7]) & 0xFFFFFFFF;
		$carry = $oneThird + $this->buffer[0];
		$this->buffer[0] = $carry & 0xFFFFFFFF;
		$carry = ($carry >> 32) + $this->buffer[1];
		$this->buffer[1] = $carry & 0xFFFFFFFF;
		...
		$this->buffer[6] = $carry & 0xFFFFFFFF;
		$this->buffer[7] = (($carry >> 32) + $this->buffer[7]) & 0xFFFFFFFF;
		$greaterThanOrEqualToPrimeMask = -(int)(((int)($this->buffer[0] < 0xFFFFFC2F) | (($this->buffer[1] ^ 0xFFFFFFFE) & 0xFFFFFFFE) | ($this->buffer[2] ^ 0xFFFFFFFF) | ($this->buffer[3] ^ 0xFFFFFFFF) | ($this->buffer[4] ^ 0xFFFFFFFF) | ($this->buffer[5] ^ 0xFFFFFFFF) | ($this->buffer[6] ^ 0xFFFFFFFF) | ($this->buffer[7] ^ 0xFFFFFFFF)) === 0);
		$lessThanPrimeMask = ~$greaterThanOrEqualToPrimeMask;
	...
	public function multiplyInteger(int $value): void {
		...
		$this->limbs[7] = (($carryPart >> 32) + $this->limbs[7]) & 0xFFFFFFFF;
		$carryPart = ($carry << 31) + $this->limbs[0];
		$this->limbs[0] = $carryPart & 0xFFFFFFFF;
		$carryPart = ($carryPart >> 32) + $this->limbs[1];
		$this->limbs[1] = $carryPart & 0xFFFFFFFF;
		...
		$this->limbs[6] = $carryPart & 0xFFFFFFFF;
		$this->limbs[7] = (($carryPart >> 32) + $this->limbs[7]) & 0xFFFFFFFF;
		$greaterThanOrEqualToPrimeMask = -(int)(((int)($this->limbs[0] < 0xFFFFFC2F) | (($this->limbs[1] ^ 0xFFFFFFFE) & 0xFFFFFFFE) | ($this->limbs[2] ^ 0xFFFFFFFF) | ($this->limbs[3] ^ 0xFFFFFFFF) | ($this->limbs[4] ^ 0xFFFFFFFF) | ($this->limbs[5] ^ 0xFFFFFFFF) | ($this->limbs[6] ^ 0xFFFFFFFF) | ($this->limbs[7] ^ 0xFFFFFFFF)) === 0);
		$lessThanPrimeMask = ~$greaterThanOrEqualToPrimeMask;
		$carry = 1 + $this->limbs[0] - 0xFFFFFC2F + 0xFFFFFFFF;
		$this->limbs[0] = (($carry & 0xFFFFFFFF) & $greaterThanOrEqualToPrimeMask) | ($this->limbs[0] & $lessThanPrimeMask);
		$carry = ($carry >> 32) + $this->limbs[1] - 0xFFFFFFFE + 0xFFFFFFFF;
